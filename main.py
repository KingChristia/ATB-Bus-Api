...
def read_root():
    ...
    if busID != "N/A":
        ...
        return {"inbound": res[:2], "geo": geo}
    return {"inbound": res[:2]}
    # print(res)
...
def getLocation(busID: str):
    ...
    if response.status_code == 200:
        ...
        for vehicle_activity in root.findall(".//siri:VehicleActivity", namespaces):
            vehicle_ref = vehicle_activity.find(
                ".//siri:VehicleMonitoringRef", namespaces
            )

            if vehicle_ref is not None and vehicle_ref.text == busID:
                vehicle_location = vehicle_activity.find(
                    ".//siri:VehicleLocation", namespaces
                )
